...
dump_token(const asn1_token_t *token, uint8_t depth, void *ctx) {
	...
	if (type->encoding == ASN1_ENCODING_CONSTRUCTED) {
		...
		case ASN1_TAG_PRINTABLESTRING:
			if (asn1_string(token, buf, sizeof(buf)).errno != ASININE_OK) {
				printf(" <INVALID>\n");
				break;
			}
		...
		case ASN1_TAG_INT: {
			...
			if (asn1_int(token, &value).errno != ASININE_OK) {
				printf(" <INVALID>\n");
				break;
			}
		...
		case ASN1_TAG_OID: {
			...
			if (asn1_oid(token, &oid).errno != ASININE_OK) {
				printf(" <INVALID>\n");
				break;
			}
		...
		case ASN1_TAG_GENERALIZEDTIME: {
			...
			if (asn1_time(token, &time).errno != ASININE_OK) {
				printf(" <INVALID>\n");
				break;
			}
		...
		case ASN1_TAG_BOOL: {
			...
			if (asn1_bool(token, &value).errno != ASININE_OK) {
				printf(" <INVALID>\n");
				break;
			}
